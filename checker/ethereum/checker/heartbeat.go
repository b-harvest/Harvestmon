	...
	for agentName, agentChecker := range c.AgentCheckers {
		...
		for _, event := range lastAgentNameAndCreatedAts {
			...
			if event.CreatedAt.Add(*maxWaitTime).Before(now) {
				...
				if alertLevelP := client.GetAlertLevel(agentName, string(HEARTBEAT_ETH_ALARM_TYPE), event.EventType); alertLevelP == nil {
					newAlertLevelP := client.GetAlertLevel(agentName, string(HEARTBEAT_ETH_ALARM_TYPE))
				...
				for _, a := range client.GetAlarmerList(agentName, alertLevel.AlertLevel) {
					...
					if err != nil {
						log.Error(errors.New(heartbeatFormatf("error occurred while sending alarm: %s, %v", HEARTBEAT_ETH_ALARM_TYPE, err)))
					}
				}
				if !sent {
					log.Error(errors.New(heartbeatFormatf("Didn't send any alert cause of no alarmer specified for the level: %s, %s", HEARTBEAT_ETH_ALARM_TYPE, alertLevel.AlertLevel)))
				}
			}
