...
func BlockCommitChecker(c *types.CheckerConfig, client *types.CheckerClient) {
	...
	for agentName, agentChecker := range c.AgentCheckers {
		...
		if len(validatorAddressesWithAgents) >= agentChecker.CommitCheck.TargetBlockCount {
			...
			log.Debug(blockCommitFormatf("Skipping to check block_commit: less than targetBlock"))
		}
